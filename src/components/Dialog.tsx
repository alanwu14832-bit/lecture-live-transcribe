...
export function Dialog({ open, onClose, title, children, footer, wide }: { open: boolean; onClose: () => void; title: string; children: ReactNode; footer?: ReactNode; wide?: boolean }) {
  ...
  return (
    <dialog
      ...
      className="dlg m-0 p-0 bg-transparent open:flex fixed inset-0 h-full max-h-full w-full max-w-full items-end sm:items-center justify-center"
    >
      <div className={`elevated-overlay w-full ${wide ? "sm:max-w-2xl" : "sm:max-w-md"} max-h-[90vh] overflow-y-auto bg-surface text-primary rounded-t-container sm:rounded-container border border-border safe-bottom`}>
