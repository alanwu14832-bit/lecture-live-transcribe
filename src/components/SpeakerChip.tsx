...
export function SpeakerChip({ index, names, onClick, size = "md" }: { index: number; names?: Record<string, string>; onClick?: () => void; size?: "sm" | "md" }) {
  ...
  const cls = `inline-flex items-center gap-1.5 rounded-full border border-border bg-surface-2 text-secondary font-medium align-middle ${size === "sm" ? "h-5 px-1.5 text-[11px]" : "h-6 px-2 text-xs"} ${onClick ? "press hover:bg-surface cursor-pointer" : ""}`;
